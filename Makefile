...
CONTAINER_REPO_OVERRIDE ?= quay.io/hyperfoil

# BUNDLE_IMG defines the image:tag used for the bundle.
# You can use it as an arg. (E.g make bundle-build BUNDLE_IMG=<some-registry>/<project-name-bundle>:<tag>)
BUNDLE_IMG ?= $(CONTAINER_REPO_OVERRIDE)/hyperfoil-operator-bundle:$(VERSION)
...
IMG ?= $(CONTAINER_REPO_OVERRIDE)/hyperfoil-operator:$(VERSION)
...
docker-buildx: test ## Build and push docker image for the manager for cross-platform support
	...
	sed -e 's/\(^FROM\)/FROM --platform=\$$\{BUILDPLATFORM\}/; t' -e '// s//FROM --platform=\$$\{BUILDPLATFORM\}/' Dockerfile > Dockerfile.cross
	- docker buildx create --driver-opt network=host --name project-v3-builder
